#---------------------------------------------------------------------------------------------------
# VECM Gibbs: regression step 
# Independent Normal-Wishart priors form Koop-Korobilis (2010)
# Matrix setting as in Koop, Leon-Gonzalez, Strachan (2008) (for n=1)
#---------------------------------------------------------------------------------------------------
    
B.SVAR.MSH.identified.regression = function(aux, priors){
   ...
   for(equation in 1:N){
      ...
      for (regime in 1:M){
         xi.m    = aux$xi[regime,]
         XX      = XX + crossprod(X[which(xi.m==1),])/lambda[equation,regime]
         XY      = XY + crossprod(X[which(xi.m==1),],Y[which(xi.m==1),])/lambda[equation,regime]
      }
