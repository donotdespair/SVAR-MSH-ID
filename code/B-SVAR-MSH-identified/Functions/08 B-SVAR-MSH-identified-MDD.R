...
B.SVAR.MSH.identified.MDD = function(Sp=1000,Gibbs.output, priors, restrictions){ #, debug=FALSE){
    ...
    for (m in 1:M){
       posteriormatrix       = cbind(posteriormatrix,t(posteriors$lambda.omega[,m,]))
    }
    ...
    for (m in 1:(M)){
       posteriormatrix       = cbind(posteriormatrix,posteriors$PR_TR[m,-M,])
    }
    posteriormatrix    = as.matrix(cbind(posteriormatrix,t(posteriors$hyper)))
    
    # Likelihood
    cat("\n 2")#;cat(class(posteriormatrix), dim(posteriormatrix), apply(posteriormatrix,2,mean))
    log.likelihood   = apply(X=posteriormatrix, MARGIN=1, FUN=loglikelihoodfordraws, restrictions=restrictions,N=N,M=M,r=r,Y=Y,XX=XX) # log-likelihood at draws
    cat("\n 2a")
    # cat("\ncomputing posterior moments and bounds")
    post.mean        = apply(posteriormatrix,2,mean)
    post.cov         = cov(posteriormatrix)
    post.cov         = 0.5*(post.cov + t(post.cov))
    lowerb           = c(rep(0,M*N), rep(-Inf,r + N*(1+N*p)), rep(0,M*(M-1) + 3) )
    upperb           = c(rep(Inf,M*N + r + N*(1+N*p)), rep(1,M*(M-1)), rep(Inf,3) )
       
    # is.draws         = rtmvnorm(n=Sp, mean = post.mean,  sigma = post.cov, lower=lowerb, upper=upperb, algorithm="gibbs", burn.in.samples=100, start.value=post.mean)
    cat("\n 3")
...
#     if (sum(id.in.range)>Sp){
#        is.draws      = is.draws[id.in.range,][1:Sp,]
#     } else {
#        # cat("\nmore is draws")
#        is.draws         = rmvnorm(n=floor(2*(length(id.in.range)/sum(id.in.range))*Sp), mean = post.mean,  sigma = post.cov)
#        is.draws      = is.draws[id.in.range,][1:Sp,]
#     }
    cat("\n 5")
    ll               = apply(X=is.draws, MARGIN=1, FUN=loglikelihoodfordraws,restrictions=restrictions,N=N,M=M,r=r,Y=Y,XX=XX) # log-likelihood at draws
    ...
    cat("Percent of IS draws in set A: ",sum(Indicator.setA)/Sp)
...
loglikelihoodfordraws = function(draws,restrictions,N,M,r,Y,XX){
   lambda.omega     = matrix(draws[1:(M*N)], ncol=M)
   a                = matrix(draws[(M*N + 1):(M*N + r)], ncol=1)
   A                = matrix(restrictions$Q %*% a + restrictions$q, ncol=N)
   A.inv            = solve(A)
   Sigma            = array(NA, c(N,N,M))
   for (m in 1:M){
      if (m==1) {
         lambda     = as.vector(lambda.omega[,1])
      } else {
         lambda     = as.vector(lambda.omega[,1]*lambda.omega[,m])
      }
      Sigma[,,m]    = A.inv %*% diag(lambda) %*% t(A.inv)
   }
   # cat("\na")
   alpha            = matrix(draws[(M*N + r + 1):(M*N + r + N*(1+N*p))], ncol=N)
   PR_TR.tmp        = matrix(draws[(M*N + r + N*(1+N*p) + 1):(M*N + r + N*(1+N*p) + (M^2 - M))], ncol=M-1, byrow=TRUE)
   PR_TR            = cbind(PR_TR.tmp, 1-apply(PR_TR.tmp,1,sum) )
   hyper            = matrix(draws[(M*N + r + N*(1+N*p) + (M^2 - M) + 1):(M*N + r + N*(1+N*p) + (M^2 - M) + 3)], ncol=1)
   # cat("\nb")
   U.SF             = Y %*% t(A) - XX %*% alpha
   U                = U.SF %*% t(A.inv)
   # cat("\nc")
   ...
   for(regime in 1:M){
      # Densities:
      eta.tmp         <- dmvnorm(U, sigma=Sigma[,,regime]) 
   ...
   # cat("\ne")
   PR.forecasted    = G.BHLK.forecasting(aux.tmp)
   output           = as.numeric(sum( log(colSums(eta.t * PR.forecasted))))
...
log.prior.for.is.draws = function(draws){
   ...
   lp.omega       = sum(dig2(x=as.vector(lambda.omega[,2:M]), a=priors$omega.a, b=priors$omega.b, logarithm=TRUE))
   lp.hyper       = sum(dig2(x=as.vector(hyper), a=priors$hyper.a, b=priors$hyper.b, logarithm=TRUE))
          
   lp.PR_TR       = 0
   for (m in 1:M){
      lp.PR_TR    = lp.PR_TR + log(ddirichlet(x=PR_TR[m,], alpha=matrix(priors$w,ncol=M)[m,]))
   }
   
   lp             = lp.mu + lp.beta + lp.a + lp.lambda + lp.omega + lp.hyper + lp.PR_TR
