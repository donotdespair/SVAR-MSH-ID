...
B.SVAR.identified.structural.MH = function(aux,priors,restrictions,nu=5,C=1){
   N  = dim(aux$Y)[2]     
   T  = dim(aux$Y)[1]     
   TT = dim(aux$U)[1]     
   p  = T - TT
   ...
   LAMBDA   = diag(1/as.vector(matrix(lambda.m[,rep(1,TT)],ncol=1)))
   ...
   if (!inherits(tmp, "try-error")){
      ...
      if (runif(1)< exp(log.kernel.candidate-log.kernel.current)) {
         aux$a    = candidate.a
         aux$A    = A0
         lambda.m    = aux$lambda.omega[,1]
         aux$Sigma   = solve(aux$A) %*% diag(lambda.m) %*% solve(t(aux$A))
...
B.SVAR.identified.A0.log.kernel = function(a,aux,priors,restrictions){
   N  = dim(aux$Y)[2]     
   T  = dim(aux$Y)[1]     
   TT = dim(aux$U)[1]     
   p  = T - TT
   ...
   for (t in 1:TT){
      ...
      u.tilde[t]     = t(y.tilde[,t] - x.tilde[,,t] %*% a) %*% diag(1/lambda.m[,1]) %*% (y.tilde[,t] - x.tilde[,,t] %*% a)
   }
